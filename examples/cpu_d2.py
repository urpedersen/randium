...
for beta in 1.3, :
    ...
    for _ in range(eq_blocks):
        lat.simulation_monte_carlo_global(eq_steps_per_blocks)
        #lat.simulation_monte_carlo_local(eq_steps_per_blocks)
        energies.append(lat.get_total_energy()/lat.N)
    print(f'Time to run equbriliation: {perf_counter()-tic} seconds')

    plt.figure()
    plt.title(f'Equbriliation {lat.L=} {lat.N=} {lat.M=} {lat.N_m=} {lat.beta=}')
    plt.plot(energies)
    ...
    inner_steps = [int(1.4 ** x) for x in range(1, 52)]
